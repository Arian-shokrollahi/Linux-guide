...
ا-**Dependency** یعنی یک Package برای اجرا یا کار کردن صحیح، به Package یا Library دیگری نیاز داشته باشد.
...
ا-Dependency های `nginx` هستند.

---

# ا-APT چطور Dependency را پیدا می‌کند؟
...
ا-APT فقط دنبال خود `nginx` نمی‌گردد.
...
ا-APT می‌تواند `libA` را هم از Repository پیدا و نصب کند:
...
# ا-APT از کجا می‌فهمد Dependency چیست؟
...
### ا-1. Dependency فقط برای نصب نیست

ا-Dependency برای این است که Package بتواند **به شکل درست کار کند**.
...
ا-APT این زنجیره Dependency را هم مدیریت می‌کند.

### ا-3. APT معمولاً Dependency ها را به‌صورت خودکار مدیریت می‌کند
...
> ا-**APT با استفاده از Dependency Information موجود در Metadata، نیازمندی‌های Package را پیدا می‌کند و در صورت نیاز آن‌ها را از Repository دریافت و نصب می‌کند.**
